...
<head>
  ...
  if(isset($_GET['tour'])){
    ...
    $page_keywords = 'экскурсии, Беларусь, познай, бел,'.implode(",", $json_ld[8]);
  } else {
    $page_title = 'Познай Бел ✪✪✪ экскурсии по Беларуси';
    $page_desc = '
    Экскурсии по Беларуси ➢ Увлекательные сборные туры. Туры на любой вкус для корпоративных и школьных групп.
    ';
  ...
  <script type="application/ld+json">
  {
    ...
    "provider": "Познай.бел",
    "serviceType": "экскурсии по Беларуси",
    "offers": {
      "@type": "AggregateOffer",
...
<body class="size-1140">
  ...
  <?php
  if(file_exists('jsdb/JSON/tours/belarus.json')){
    $file = json_decode(file_get_contents('jsdb/JSON/tours/belarus.json'))[0];
    $check_value = false;
    ...
    if($check_value){
      include 'includes/belarus/all_tours.php';
    }
  }
  if(file_exists('jsdb/JSON/tours/belarus_pref.json')){
    $file = json_decode(file_get_contents('jsdb/JSON/tours/belarus_pref.json'))[0];
    $check_value = false;
    foreach($file as $key => $value){
      if(array_key_exists(1,$value)){
        $check_value = true;
    ...
    if($check_value){
      include 'includes/belarus/all_tours_pref.php';
    }
  }
  include 'includes/belarus/how_work.php';
  include 'includes/belarus/search_tour.php';
  if(isset($_GET['tour'])){
    include 'includes/belarus/_schema.php';
  }
  ?>
  ...
  new TabsTour('belarus_pref_tours');
  new SearchTour('belarus');
  </script>
</body>
</html>
