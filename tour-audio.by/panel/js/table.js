class Table{
	...
	createObjectCols(item,set,i,empty){
		...
		for(let key in item){
			let col;
			if(set.items[key]){
				let width = set.itemsSize && set.itemsSize[key] ? set.itemsSize[key] : false;
				let typeCol = this.getTypeCol(set,key);
				switch(typeCol){
					case 'text':
					col = this.createText(set.type,empty ? '' : item[key], width);
					wrap.appendChild(col);
					break;
					case 'singleImg':
					col = this.createSingleImg(item,set,empty ? '' : item[key], width);
					wrap.appendChild(col);
					break;
					case 'sliderImg':
					col = this.createSliderImg(item.imgPath,empty ? '' : item[key], width);
					wrap.appendChild(col);
					break;
					case 'dateMultiRange':
					col = this.createDateMultiRangeCol(empty ? '' : item[key], width);
					wrap.appendChild(col);
					break;
					case 'operator':
					col = this.createOperatorCol(empty ? '' : item[key],i, width);
					wrap.appendChild(col);
					break;
					case 'messenger':
					col = this.createMessengerCol(empty ? '' : item[key],i, width);
					wrap.appendChild(col);
					break;
					case 'social':
					col = this.createSocialCol(empty ? '' : item[key],i, width);
					wrap.appendChild(col);
					break;
				}
			}
	...
	createStringCols(item,set,i,empty){
		...
		switch(typeCol){
			case 'text':
			col = this.createText(set.type,empty ? '' : item, set.width);
			break;
			case 'singleImg':
			col = this.createSingleImg(item,set,empty ? '' : item, set.width);
			break;
			case 'sliderImg':
			col = this.createSliderImg(set.type,empty ? '' : item, set.width);
			break;
			case 'dateMultiRange':
			col = this.createDateMultiRangeCol(empty ? '' : item[key], set.width);
			break
		}
	...
	createSingleImg(item,set,imgName,width){
	...
	createTxtCol(set){
		...
		col.appendChild(p);
		if(set.width){
			col.style.width = set.width;
			p.style.width = set.width;
		}
		return col;
	}
	createBtnCol(set){
	...
	createSingleImgCol(set){
		...
		if(set.src){
			...
			if(set.width){
				img.style.width = set.width;
				openModal.style.width = set.width;
				wrap.style.width = set.width;
				col.style.width = set.width;
			}
		}
