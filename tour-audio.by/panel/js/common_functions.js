class CommonFunctions{
	...
	writeJsonRequest(fun,path,json){
		let scroll = window.scrollY;
		let jsonFile = json ? JSON.stringify(json) : JSON.stringify(this.json);
		let jsonPath = path ? encodeURIComponent(path) : encodeURIComponent(this.set.json);
		this.sendRequest({
			'method':'POST',
			'url':this.set.request_file+'?action=write_json&path='+jsonPath,
			'dates':'file='+jsonFile
		}).then((()=>{
			if(fun) fun();
